...
pub enum Node {
    ...
    StmtLet { name: String, value: Expression },
...
impl Parser {
    fn next_token(&mut self) -> Token {
        if self.current.kind == Eof && self.peek.kind == Eof {
        ...
        let res = match self.current.clone() {
            ...
            Token { kind: TokenKind::Let, .. } => {
                ...
                match (ident, equals, expr) {
                    ...
                    (ident, assign, expr) => StmtError { current: ident.kind, peek: assign.kind }
                }
            }
    ...
    fn next_prefix(&mut self, precedence: Precedence) -> Expression {
        let mut left = match self.current.clone().kind {
            ...
            _ => ExprError {
                value: vec![self.current.clone().kind],
            }
        };

        while self.peek.kind != Semicolon && precedence < Precedence::from(&self.peek.kind) {
    ...
    fn next_infix(&mut self, left: Expression) -> Expression {
        match ExprKind::try_from(&self.peek.kind) {
            Ok(kind) => {
...
mod tests {
    ...
    fn parser_initialization() {
        ...
        parser.next_token();
        assert_eq!(parser.current.kind, TokenKind::Int { value: 1 });
        assert_eq!(parser.peek.kind, TokenKind::Plus);

        parser.next_token();
        assert_eq!(parser.current.kind, TokenKind::Plus);
        assert_eq!(parser.peek.kind, TokenKind::Int { value: 2 });

        parser.next_token();
        assert_eq!(parser.current.kind, TokenKind::Int { value: 2 });
        assert_eq!(parser.peek.kind, TokenKind::Eof);
    }
    ...
    fn infix_precedence() {
        ...
        assert_eq!(
            ...
            StmtExpr {
                value: ExprBinary {
                    kind: ExprKind::Times,
                    left: Box::from(ExprPrefix {
                        kind: PrefixKind::Minus,
                        value: ExprIdent { name: "a".into() }.into(),
                    }),
                    right: Box::from(ExprIdent {
                        name: "b".into()
                    }),
                }
            }
            ...
            StmtExpr {
                value: ExprBinary {
                    left: Box::from(ExprBinary {
                        kind: ExprKind::Plus,
                        left: ExprIdent { name: "a".into() }.into(),
                        right: ExprIdent { name: "b".into() }.into(),
                    }),
                    kind: ExprKind::Plus,
                    right: Box::from(ExprIdent { name: "c".into() }
                    ),
                }
            }
        );
